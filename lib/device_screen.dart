...
class _DeviceScreenState extends State<DeviceScreen> with TickerProviderStateMixin{
  ...
  initState() {
    ...
    eventBus.on().listen((event) {
      ...
      if(event == "4"){
        print("현재 블루투스 상태 : " + state.toString());
        setState(() { });
      }
      if(event == "5"){
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: appBar("코딩 조작", chick.bleState, context,true),
  ...
  //   );
  // }
  Widget Listview_sperated(Chick chick, BuildContext context, int index) {
    ...
    return Transform(
      // alignment: Alignment.center,
        transform: Matrix4.identity()..translate(radius * math.cos(radians(index*-36)),radius*math.sin(radians(index*-36))),
        child:ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
            ...
            fixedSize: Size(MediaQuery.of(context).size.width * 0.07, MediaQuery.of(context).size.width * 0.07,),
            shape: CircleBorder(),
          ),
        )
...
Widget _Blocks(BuildContext context, int col,AnimationController _rotationController,Animation<double> _rotationAnimation){
  ...
  return Container(
    ...
    width: (MediaQuery.of(context).size.width) * 0.90 -50,
    child: ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return Container(
          width: (MediaQuery.of(context).size.height-40) * 0.12,
          color: Colors.transparent,
          child: Center(
